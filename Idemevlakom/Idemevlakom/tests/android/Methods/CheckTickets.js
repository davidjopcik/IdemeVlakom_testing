...
class CheckTickets {
    ...
    async ChecktrainTimeDate(userData) {
        ...
        trainDataCheck.trainFrom = await AddPassenger.FromResult.getText()
        trainDataCheck.trainTo = await TrainToInSegment.getText()
    ...
    async CheckTicket(userData, trainData) {
        ...
        while(await timeDepartureInTicket !== await i.trainTimeDeparture){
        ...
        await browser.saveScreenshot("./Idemevlakom/tests/android/testcases/Screenshots/"+i.trainFrom + " - " + i.trainTo+".png")
        j += 1
    ...
    async CheckName(i) {
        ...
        if(!await $('//*[@text="ZObraziť doklady"]')){
            await Swipe.swipeElement0nTop($('//*[@text="Cestujúci"]'))
        }
        passengerItemName = await $('//*[@resource-id="sk.zssk.mobapp.android.dev:id/f_route_passengers_item_name"]').getText()
